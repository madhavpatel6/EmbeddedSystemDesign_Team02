...
//            computation_threadData.alignmentResult = determineAlignment(&obj);
...
int determineAlignment(bool ir_0_bool, bool ir_1_bool, bool ir_2_bool, float ir_0, float ir_1, float ir_2){
    // I need to figure out how i want to determine if i can grab the target
    // or not
    // Returning 2 now for testing
    if((ir_0_bool == true) && (ir_1_bool == true) && (ir_2_bool == true)){
        if((ir_0 == ir_1) && (ir_1 == ir_2)){ // This will need some error tolerance
            if(ir_1 < 4.0){ // tells that theyre close enough to grab
                return 2;
            }
            else{
                return 1; // Too far away to grab
            }
        }
        else{ 
            return 0; // current values are good but not aligned
        }
    }
    else {
        return 0; // latest values were not good, not aligned
    }
}
